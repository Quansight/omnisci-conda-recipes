...
CONDA_PREFIX=${CONDA_PREFIX:=$(dirname $(dirname $(which conda)))}

descs["MAPD_PATH"]="MapD install directory"
vars["MAPD_PATH"]=${MAPD_PATH:=$CONDA_PREFIX}
...
for f in mapd_server mapd_server@ mapd_sd_server mapd_sd_server@ mapd_web_server mapd_web_server@ ; do
  if [ -f $MAPD_PATH/systemd/$f.service.in ]; then
    sed -e "s#@MAPD_PATH@#${vars['MAPD_PATH']}#g" \
        ...
        $MAPD_PATH/systemd/$f.service.in > $MAPD_TMP/$f.service
    sudo cp $MAPD_TMP/$f.service /lib/systemd/system/
  fi
done
if [ -f $MAPD_PATH/systemd/mapd_xorg.service ]; then
	sudo cp $MAPD_PATH/systemd/mapd_xorg.service /lib/systemd/system/
fi
...
sed -e "s#@MAPD_PATH@#${vars['MAPD_PATH']}#g" \
    ...
    -e "s#@MAPD_USER@#${vars['MAPD_USER']}#g" \
    -e "s#@MAPD_GROUP@#${vars['MAPD_GROUP']}#g" \
    $MAPD_PATH/systemd/mapd.conf.in > $MAPD_TMP/mapd.conf
if [ -f $MAPD_PATH/systemd/mapd-sds.conf.in ]; then
  sed -e "s#@MAPD_PATH@#${vars['MAPD_PATH']}#g" \
      -e "s#@MAPD_STORAGE@#${vars['MAPD_STORAGE']}#g" \
      -e "s#@MAPD_DATA@#${vars['MAPD_DATA']}#g" \
      -e "s#@MAPD_USER@#${vars['MAPD_USER']}#g" \
      -e "s#@MAPD_GROUP@#${vars['MAPD_GROUP']}#g" \
      $MAPD_PATH/systemd/mapd-sds.conf.in > $MAPD_TMP/mapd-sds.conf
